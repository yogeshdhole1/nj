	...
	function generate_post_select($select_id, $post_type, $selected = 0) {
 	...
 	function form( $instance ) {
		...
		for( $i=0; $i<3; $i++) { 
			?>
			<p>
				<label for="<?php echo $this->get_field_id( key($defaults) ); ?>"><?php _e( 'Post', 'spacious' ); ?>:</label>
				<?php 
				$this->generate_post_select($this->get_field_name( key($defaults) ), 'post', $instance[key($defaults)]);
				
				//wp_dropdown_pages( array( 'show_option_none' =>' ','name' => $this->get_field_name( key($defaults) ), 'selected' => $instance[key($defaults)] ) ); ?>
			</p>
		<?php
		next( $defaults );// forwards the key of $defaults array
		}
