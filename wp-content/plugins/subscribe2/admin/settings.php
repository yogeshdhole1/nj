...
if ( isset( $_POST['s2_admin']) ) {
	...
	if ( isset($_POST['reset']) ) {
		...
		foreach ($_POST as $key => $value) {
			...
			} elseif ( in_array($key, array('notification_subject', 'notification_subject_update', 'mailtext', 'mailtext_update', 'confirm_subject', 'confirm_email', 'remind_subject', 'remind_email')) && !empty($_POST[$key]) ) {
...
switch ($current_tab) {
	...
	case 'templates':
		...
		echo "</dl></td></tr>";
		echo "<tr><td style=\"vertical-align: top; height: 350px; min-height: 350px;\">";
		echo __('Notification email for updated posts (must not be empty)', 'subscribe2') . ":<br />\r\n";
		echo __('Subject Line', 'subscribe2') . ": ";
		echo "<input type=\"text\" name=\"notification_subject_update\" value=\"" . stripslashes($this->subscribe2_options['notification_subject_update']) . "\" size=\"45\" />";
		echo "<br />\r\n";
		echo "<textarea rows=\"9\" cols=\"60\" name=\"mailtext_update\">" . stripslashes($this->subscribe2_options['mailtext_update']) . "</textarea>\r\n";
		echo "</td></tr>";
