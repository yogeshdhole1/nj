...
/* ===== mail handling ===== */
	...
	*/
	function publish($post, $preview = '') {
		...
		// Get email subject
		$sub = 'notification_subject';
		$mailt = 'mailtext';
		if ($post->post_date != $post->post_modified) {
			$sub = 'notification_subject_update';
			$mailt = 'mailtext_update';
		  }
		  
		$subject = html_entity_decode(stripslashes(wp_kses($this->substitute($this->subscribe2_options[$sub]), '')));
		// Get the message template
		$mailtext = apply_filters('s2_email_template', $this->subscribe2_options[$mailt]);
...
/* ===== Registered User and Subscriber functions ===== */
	...
	*/
	function register($user_ID = 0, $consent = false, $cats = '') {
		global $wpdb;
...
/* ===== helper functions: forms and stuff ===== */
	...
	*/
	...
	Process function to add action if user selects to subscribe to posts during registration
	*/
	function wpmembers_register_post($user_ID = 0) {
		$user_ID = (isset($user_ID['ID']))?$user_ID['ID']:0;
		global $_POST;
		if ( 0 == $user_ID ) { return; }
		if ( 'yes' == $this->subscribe2_options['autosub'] || ( isset($_POST['reg_subscribe']) && 'on' == $_POST['reg_subscribe'] && 'wpreg' == $this->subscribe2_options['autosub'] ) ) {
			$cats = isset($_POST['category'])?implode(',', $_POST['category']):'';
			$this->register($user_ID, true, $cats);
		} else {
			$this->register($user_ID, false);
		}
	} // end register_post()
	...
	*/
	function s2init() {
		...
		if ( $this->s2_mu ) {
			...
			add_action('wpmem_post_register_data', array(&$this, 'wpmembers_register_post'));
		}
		...
		if ( $this->subscribe2_options['email_freq'] != 'never' ) {
			...
			$statuses = apply_filters('s2_post_statuses', array('new', 'publish',  'draft', 'auto-draft', 'pending'));
			if ( $this->subscribe2_options['private'] == 'yes' ) {
...
//		add_action('admin_init', array(&$this, 'on_plugin_activated_redirect'));
		...
		if ( is_admin() ) {
			...
			add_shortcode('subscribe2_wpmembers', array(&$this, 'shortcode_wpmembers'));
