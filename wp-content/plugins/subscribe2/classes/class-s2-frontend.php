...
/* ===== template and filter functions ===== */
	...
	*/
	...
	/**
	 * Display our checkbox; also handles (un)subscribe requests
	 *
	 * Author Nitin Singh on 21-July-2014
	 */
	function shortcode_wpmembers($atts) {
		$args = shortcode_atts(array(
			'hide'  => '',
			'id'    => '',
			'nojs' => 'false',
		...
		wp_register_script('s2_checkbox_wpmembers', S2URL . 'include/s2_checkbox_wpmembers.js', array('jquery'), '1.3');
		wp_enqueue_script('s2_checkbox_wpmembers');	
			
		if ( 'no' == $this->subscribe2_options['autosub'] ) { return; }
		if ( 'wpreg' == $this->subscribe2_options['autosub'] ) {
			$this->s2form = "<div class=\"div_text\">";
			$this->s2form .= "<input calss=\"reg_subscribe\" type=\"checkbox\" name=\"reg_subscribe\"" . checked($this->subscribe2_options['wpregdef'], 'yes', false) . " value=\"on\">&nbsp;&nbsp;&nbsp;";
			$this->s2form .= __('Subscribe to email notifications for posts related to below categories', 'subscribe2') . "";
			('' == $this->subscribe2_options['compulsory']) ? $compulsory = array() : $compulsory = explode(',', $this->subscribe2_options['compulsory']);
			
			$s2_admin = new s2_admin;
			$this->s2form .= $s2_admin->get_category_form(explode(',', get_user_meta($user_ID, $this->get_usermeta_keyname('s2_subscribed'), true)), $this->subscribe2_options['reg_override'], $compulsory);
			$this->s2form .= "</div>";
		} elseif ( 'yes' == $this->subscribe2_options['autosub'] ) {
			$this->s2form = "<div class=\"div_text\">";
			$this->s2form .= __('By registering with this blog you are also agreeing to receive email notifications for new posts but you can unsubscribe at anytime', 'subscribe2') . ".<br />\r\n";
			$this->s2form .= "</div>";
		}
