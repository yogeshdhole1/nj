...
/* ===== WordPress menu registration and scripts ===== */
	...
	*/
	function admin_menu() {
		add_menu_page(__('Subscription', 'subscribe2'), __('Subscription', 'subscribe2'), apply_filters('s2_capability', "read", 'user'), 's2', NULL, S2URL . 'include/email_edit.png');
...
/* ===== WordPress menu helper functions ===== */
	...
	*/
	function get_category_form($selected = array(), $override = 1, $compulsory = array(), $name = 'category') {
		global $wpdb;
		...
		$j = 0;
		$field_list = '';
		$field_list = "<div class=\"div_text cat_list\" style=\"margin-left: 25px;margin-top: 10px\">";
		...
		foreach ( $all_cats as $cat ) {
			...
			if ( 0 == $j ) {
				$field_list .= "<input class=\"checkall_" . $name . "\" type=\"checkbox\" name=\"" . $name . "[]\" value=\"" . $cat->term_id . "\"";
				if ( in_array($cat->term_id, $selected) || in_array($cat->term_id, $compulsory) || true ) {
					$field_list .= " checked=\"checked\"";
				}
				if ( in_array($cat->term_id, $compulsory) && $name === 'category' ) {
					$field_list .= " DISABLED";
				}
				$field_list .= " /> <abbr title=\"" . $cat->slug . "\">" . $catName . "</abbr><br />\r\n";
			} else {
				$field_list .= "<input class=\"checkall_" . $name . "\" type=\"checkbox\" name=\"" . $name . "[]\" value=\"" . $cat->term_id . "\"";
				if ( in_array($cat->term_id, $selected) || in_array($cat->term_id, $compulsory)  || true ) {
					$field_list .= " checked=\"checked\"";
		...
		if ( !empty($compulsory) ) {
			foreach ($compulsory as $cat) {
				$field_list .= "<input type=\"hidden\" name=\"" . $name . "[]\" value=\"" . $cat . "\">\r\n";
			}
		}
		$field_list .= "</div>";
		return $field_list;
